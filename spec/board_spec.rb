...
RSpec.describe Board do
  ...
  describe '#white_pawn_search' do
    ...
    context 'when a white pawn is blocked from moving forward' do
      pawn = Pawn.new('W', 'D2')
      origin_ary = [6, 3]
      ...
      it 'returns nil for blocked path for double-step' do
        target_ary = [4, 3]
        board.grid[5][3] = Knight.new
        result = board.white_pawn_search(origin_ary, pawn, target_ary)
        expect(result).to be_nil
      end
    end
  ...
  describe '#black_pawn_search' do
    ...
    context 'when a black pawn is block from moving forward' do
      pawn = Pawn.new('B', 'A7')
      origin_ary = [1, 0]
  ...
  describe '#checkmate?' do
    ...
    enemy_king = King.new('W', 'H6')
    
    it 'returns true when the black king is mated' do
      board.grid[0][7] = king
      board.grid[0][4] = enemy_rook
      board.grid[2][7] = enemy_king
